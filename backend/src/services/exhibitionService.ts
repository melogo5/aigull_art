...
export class ExhibitionService {
  static async listExhibitions(
    filters: Partial<IExhibition>
  ): Promise<IExhibition[]> {
    ...
    if (pictures) {
      const ids = Array.isArray(pictures)
        ? pictures
        : String(pictures).split(',');
      query.pictures = {
        $in: ids.filter(Boolean).map((id: string) => new Types.ObjectId(id)),
      } as any;
    }
  ...
  static async editExhibition(
    id: string,
    updateData: Partial<IExhibition>
  ): Promise<IExhibition | null> {
    if ((updateData as any).startDate)
      (updateData as any).startDate = new Date(
        (updateData as any).startDate as any
      );
    if ((updateData as any).endDate)
      (updateData as any).endDate = new Date(
        (updateData as any).endDate as any
      );
    if ((updateData as any).pictures) {
      (updateData as any).pictures = (updateData as any).pictures.map(
        (id: string) => new Types.ObjectId(id)
      );
    }
    return await Exhibition.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
  }
